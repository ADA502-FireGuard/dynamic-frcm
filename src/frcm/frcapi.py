...
class FireRiskAPI:
    ...
    def compute_from_raw_data(self, temp: float, temp_forecast: float, humidity: float, humidity_forecast: float, wind_speed: float, wind_speed_forecast: float, timestamp: str, timestamp_forecast: str, long: float, lat: float) -> FireRiskPrediction:
        # Define location
        location = Location(latitude=lat, longitude=long)

        # Define observations
        wdp_observation = WeatherDataPoint(temperature=temp, humidity=humidity, wind_speed=wind_speed, timestamp=timestamp)
        data_observations = list[wdp_observation]
        observations = Observations(location=location, data=data_observations)
        ...
        data_forecast = list[wdp_forecast]
